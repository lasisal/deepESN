function [inputSequence, outputSequence] = generate_NARMA_sequence_m(sequenceLength, memoryLength, nInputUnits, corrMat)
...
%%%% create input 
inputSequence = rand(sequenceLength,nInputUnits);
inputSequence = inputSequence * corrMat/sum(corrMat(1,:));
...
for i = memoryLength + 1 : sequenceLength
    ...
    % example
    outputSequence(i,1) = 0.7 * sum(inputSequence(i,:))/nInputUnits + 0.1 ...

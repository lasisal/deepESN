...
predictedTrainOutput = zeros(500,seriesNum+1);
predictedTrainOutput(:,1) = trainInputSequence; 
predictedTestOutput = zeros(500,seriesNum+1);
predictedTestOutput(:,1) = testInputSequence;
...
[trainedEsn stateMatrix] = ...
    train_esn(predictedTrainOutput(:,i), trainOutputSequence, esn, nForgetPoints) ; 
...
predictedTrainOutput(:,i+1) = test_esn(trainInputSequence, trainedEsn, nForgetPoints);
predictedTestOutput(:,i+1) = test_esn(testInputSequence,  trainedEsn, nForgetPoints) ; 
end

nForgetPoints = 100;
totPredictedTrainOutput = predictedTrainOutput(nForgetPoints+1:end,seriesNum+1);
totPredictedTestOutput = predictedTestOutput(nForgetPoints+1:end,seriesNum+1);

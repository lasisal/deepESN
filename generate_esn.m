...
%                                                connectivity);
esn.internalWeights_UnitSR = generate_internal_SCR_weights(nInternalUnits);                                        
esn.nTotalUnits = nTotalUnits; 

% input weight matrix has weight vectors per input unit in colums
esn.inputWeights = 2.0 * rand(nInternalUnits, nInputUnits)- 1.0;%(-1,1)
...
esn.feedbackWeights = (2.0 * rand(nInternalUnits, nOutputUnits)- 1.0);%(-1,1)
...
esn.noiseLevel = 0.0 ; 
esn.reservoirActivationFunction = 'identity';
